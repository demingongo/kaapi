...
import { object, type ObjectEntries, type AnySchema } from 'valibot';
...
export abstract class BaseValibotHelper implements BaseHelperInterface {
    ...
    constructor({ value: schema = {} }: { value?: object | unknown; isRoot?: boolean }, isRequired?: boolean) {
        ...
        if (schema && typeof schema === 'object') {
            if ('type' in schema && schema.type === 'instance' && 'kind' in schema && schema.kind === 'schema') {
    ...
    hasMin(): boolean {
        return (
            'minProperties' in this._schema ||
            'minItems' in this._schema ||
            'minimum' in this._schema ||
            'minLength' in this._schema
        );
    }
    hasMax(): boolean {
        return (
            'maxProperties' in this._schema ||
            'maxItems' in this._schema ||
            'maximum' in this._schema ||
            'maxLength' in this._schema
        );
    }
 ...
 * Valibot helper for \@novice1/api-doc-generator
 */
export class OpenAPIValibotHelper
    extends BaseValibotHelper
    implements OpenAPIHelperInterface, KaapiOpenAPIHelperInterface
{
    ...
    getFilesChildren(): Record<string, unknown> {
        ...
        if (vSchema && 'entries' in vSchema && typeof vSchema.entries === 'object' && vSchema.entries) {
            ...
            for (const p in properties) {
                ...
                if (ch.isValid() && ch.isFile()) r[p] = ch.getRawSchema() as ObjectEntries[string];
            }
        }
