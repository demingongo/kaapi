...
export class ClientSecretPost implements ClientAuthMethod {
    ...
    extractParams(req: Request<ReqRefDefaults>): ClientAuthMethodResponse {
        ...
        if (
            ...
            res.hasAuthMethod = true;
            if (typeof req.payload.client_id === 'string') res.clientId = req.payload.client_id;
            if (typeof req.payload.client_secret === 'string') res.clientSecret = req.payload.client_secret;
        }
...
export class NoneAuthMethod implements ClientAuthMethod {
    ...
    extractParams(req: Request<ReqRefDefaults>): ClientAuthMethodResponse {
        ...
        if (req.payload && typeof req.payload === 'object' && 'client_id' in req.payload) {
            res.hasAuthMethod = true;
            if (typeof req.payload.client_id === 'string') res.clientId = req.payload.client_id;
        }
    ...
    async extractParams(req: Request<ReqRefDefaults>): Promise<ClientAuthMethodResponse> {
        ...
        if (
            ...
            if (decoded.aud && typeof decoded.aud === 'string') {
                ...
                const clientSecret = await this.#handler(decoded.aud, decoded, req.payload.client_assertion);
...
export class PrivateKeyJwt implements ClientAuthMethod {
    ...
    ) => Promise<CryptoKey | KeyObject | JWK | Uint8Array | string | null>;
    ...
    async extractParams(req: Request<ReqRefDefaults>): Promise<ClientAuthMethodResponse> {
        ...
        if (
            ...
            if (decoded.aud && typeof decoded.aud === 'string') {
                res.clientId = decoded.aud;
                const publicKey = await this.#handler(decoded.aud, decoded, req.payload.client_assertion);
