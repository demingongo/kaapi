...
export type TokenTypeValidation<Refs extends ReqRef = ReqRefDefaults> = (
    req: Request<Refs>,
    token: string,
    ttl: number
) => TokenTypeValidationResponse | Promise<TokenTypeValidationResponse>;

export type TokenRequestValidation<Refs extends ReqRef = ReqRefDefaults> = (
    req: Request<Refs>,
    ttl: number
) => TokenTypeValidationResponse | Promise<TokenTypeValidationResponse>;
...
export class BearerToken<Refs extends ReqRef = ReqRefDefaults> implements IBearerToken<Refs> {
    #ttl: number = 300;
...
export class DPoPToken<Refs extends ReqRef = ReqRefDefaults> implements IDPoPToken<Refs> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    private async _handleDefault(req: Request<any>, ttl: number): Promise<TokenTypeValidationResponse> {
        ...
        try {
            ...
            const fullUrl = protocol + '://' + req.info.host + req.path;
