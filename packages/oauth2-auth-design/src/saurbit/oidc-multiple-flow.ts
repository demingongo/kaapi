 ...
 */
export interface KaapiOIDCFlow<Refs extends ReqRef = ReqRefDefaults> extends OIDCFlow, KaapiOIDCAdapted<Refs> {}
 ...
 * @template Refs - The Kaapi `ReqRef` type for the application.
 */
export class KaapiOIDCMultipleFlows<Refs extends ReqRef = ReqRefDefaults> extends OIDCMultipleFlows<
    KaapiOIDCFlow<Refs>
> {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
            ...
            return async (request, h) => {
                ...
                for (const [i, middleware] of middlewares.entries()) {
                    try {
                        ...
                        if (middlewares.length - 1 === i || !Boom.isBoom(error, 401)) {
                            throw error;
                        }
                    }
                }
 ...
 * @template Refs - The Kaapi `ReqRef` type for the application.
 */
export type KaapiOIDCMultipleFlowsBuilderArgs<Refs extends ReqRef = ReqRefDefaults> = Omit<
    Partial<KaapiOIDCMultipleFlowsArgs<Refs>>,
    'flows'
>;
...
export class KaapiOIDCMultipleFlowsBuilder<Refs extends ReqRef = ReqRefDefaults> implements KaapiOIDCFlowBuilder {
    ...
    static create<Refs extends ReqRef = ReqRefDefaults>(
        args?: KaapiOIDCMultipleFlowsBuilderArgs<Refs>
    ): KaapiOIDCMultipleFlowsBuilder<Refs> {
