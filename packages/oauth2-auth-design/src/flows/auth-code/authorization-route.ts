...
> implements IOAuth2ACAuthorizationRoute<GetRefs, PostRefs> {
    static buildDefault<GetRefs extends ReqRef = ReqRefDefaults, PostRefs extends ReqRef = ReqRefDefaults>() {
...
> extends OAuth2ACAuthorizationRoute<GetRefs, PostRefs> {
    ...
    constructor() {
        super(
            '/oauth2/authorize',
            async ({ clientId, redirectUri, ...props }, req, h) => {
                const validationError = await this.validateClientParams(
                    clientId,
                    redirectUri,
    ...
    setUsernameField(value: string): this {
        ...
        if (escaped) this.#usernameField = escaped;
        return this;
    }

    setPasswordField(value: string): this {
        const escaped = encode(value); // For HTML rendering, use encode() (from html-entities)
        if (escaped) this.#passwordField = escaped;
        return this;
    }
}
