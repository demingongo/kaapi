...
import {
    DefaultOAuth2ACTokenRoute,
    IOAuth2ACTokenRoute,
    OAuth2ACTokenParams,
    OAuth2ACTokenRoute,
} from './auth-code/token-route';
import {
...
export class OAuth2AuthorizationCode extends OAuth2AuthDesign implements OAuth2SingleAuthFlow {
    ...
    constructor({ authorizationRoute, tokenRoute, refreshTokenRoute, ...props }: OAuth2AuthorizationCodeArg) {
        ...
        const sr: {
            ...
            handle: async (req, h) => {
                ...
                if (
                    clientId &&
                    req.payload.code &&
                    typeof req.payload.code === 'string' &&
...
export class OAuth2AuthorizationCodeBuilder implements OAuth2AuthDesignBuilder {
    ...
    authorizationRoute<GetRefs extends ReqRef = ReqRefDefaults, PostRefs extends ReqRef = ReqRefDefaults>(
        handler: (route: DefaultOAuth2ACAuthorizationRoute<GetRefs, PostRefs>) => void
    ): this {
    ...
    refreshTokenRoute<Refs extends ReqRef = ReqRefDefaults>(
        handler: (route: DefaultOAuth2RefreshTokenRoute<Refs>) => void
    ): this {
