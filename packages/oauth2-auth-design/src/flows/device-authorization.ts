...
import {
    DefaultOAuth2DeviceAuthTokenRoute,
    IOAuth2DeviceAuthTokenRoute,
    OAuth2DeviceAuthTokenParams,
    OAuth2DeviceAuthTokenRoute,
    OAuth2DeviceCodeTokenErrorBody,
} from './device-auth/token-route';
import { KaapiTools, Lifecycle, ReqRef, ReqRefDefaults, Request, ResponseToolkit, RouteOptions } from '@kaapi/kaapi';
import { ClientAuthentication, OAuth2Util } from '@novice1/api-doc-generator';
import { BaseAuthUtil } from '@novice1/api-doc-generator/lib/utils/auth/baseAuthUtils';

const GRANT_TYPE = 'urn:ietf:params:oauth:grant-type:device_code';

export type DefaultOAuth2DeviceAuthRefreshTokenRoute<Refs extends ReqRef = ReqRefDefaults> =
    DefaultOAuth2RefreshTokenRoute<Refs, OAuth2DeviceCodeTokenErrorBody>;
        ...
        const sr: {
            ...
            handle: async (req, h) => {
                ...
                if (req.payload.client_id && typeof req.payload.client_id === 'string') {
                    ...
                    return h
                        .response({ error: OAuth2ErrorCode.INVALID_REQUEST, error_description: errorDescription })
                        .code(400);
                }
            },
        };
    ...
    async handleToken<Refs extends ReqRef = ReqRefDefaults>(
        ...
        const sr: {
            ...
            handle: async (req, h) => {
                ...
                const { clientId, clientSecret, error, errorDescription } = await this._extractClientParams(
                    req as unknown as Request<ReqRefDefaults>,
                    authMethodsInstances,
                    supported
                );
                ...
                if (
                    clientId &&
                    req.payload.device_code &&
                    typeof req.payload.device_code === 'string' &&
                    ...
                    const ttR: TokenTypeValidationResponse = tokenTypeInstance.isValidTokenRequest
                        ? await tokenTypeInstance.isValidTokenRequest(req)
                        : { isValid: true };
                    if (!ttR.isValid) {
                        return h
                            .response({ error: OAuth2ErrorCode.INVALID_REQUEST, error_description: ttR.message || '' })
                            .code(400);
                    }
    ...
    async handleRefreshToken<Refs extends ReqRef = ReqRefDefaults>(
        ...
        const sr: {
            ...
            handle: async (req, h) => {
                ...
                if (clientId && hasRefreshToken && isRefreshTokenGrantType) {
                    const scope =
                        req.payload.scope && typeof req.payload.scope === 'string' ? req.payload.scope : undefined;
...
export class OIDCDeviceAuthorization extends OAuth2DeviceAuthorization implements OAuth2SingleAuthFlow {
    ...
    constructor(params: OIDCDeviceAuthorizationArg) {
        ...
        if (params.openidConfiguration) this.openidConfiguration = params.openidConfiguration;
    }
...
export class OAuth2DeviceAuthorizationBuilder implements OAuth2AuthDesignBuilder {
    ...
    authorizationRoute<PostRefs extends ReqRef = ReqRefDefaults>(
        handler: (route: DefaultOAuth2DeviceAuthorizationRoute<PostRefs>) => void
    ): this {
        handler(this.params.authorizationRoute);
        return this;
    }

    tokenRoute<Refs extends ReqRef = ReqRefDefaults>(
        handler: (route: DefaultOAuth2DeviceAuthTokenRoute<Refs>) => void
    ): this {
    ...
    refreshTokenRoute<Refs extends ReqRef = ReqRefDefaults>(
        handler: (route: DefaultOAuth2DeviceAuthRefreshTokenRoute<Refs>) => void
    ): this {
