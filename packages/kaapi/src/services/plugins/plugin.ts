...
export abstract class AuthDesign implements KaapiPlugin {
    async integrate(t: KaapiTools): Promise<void> {
        ...
        if (securityScheme) {
            ...
            t.openapi
                ?.addSecuritySchemeAliases(securityScheme, [...new Set([this.getStrategyName()].flatMap((v) => v))])
...
export class GroupAuthDesign extends AuthDesign {
    ...
    docs() {
        return new KaapiGroupAuthUtil(this.designs.map((d) => d.docs()).filter((d) => typeof d != 'undefined'));
    }
    ...
    async integrateHook(t: KaapiTools): Promise<void> {
        await Promise.all(this.designs.map((d) => d.integrateHook(t)));
    }
