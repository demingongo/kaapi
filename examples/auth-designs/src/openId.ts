...
export const openIDDesign = new OIDCAuthorizationCode({
    ...
    authorizationRoute: new OAuth2ACAuthorizationRoute(
        ...
        (async (
            ...
            { query: { nonce }, payload: { email, password } },
            h
        ) => {
            console.log('clientId', clientId);
            console.log('codeChallenge', codeChallenge);
    ...
    tokenRoute: new OAuth2ACTokenRoute('/oauth2/ac/token', (async (
        { clientId, clientSecret, code, codeVerifier, redirectUri, createIdToken },
        _req,
        h
    ) => {
