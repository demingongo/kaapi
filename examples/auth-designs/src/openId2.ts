...
export const openIDDesign2 = new OIDCAuthorizationCode({
    ...
    tokenRoute: OAuth2ACTokenRoute.buildDefault()
        ...
        .generateToken(
            async ({ clientId, clientSecret, code, codeVerifier, redirectUri, ttl, createIdToken }, _req) => {
    ...
    refreshTokenRoute: new OAuth2RefreshTokenRoute('/oauth2/ac/token', (async (
        { clientId, clientSecret, refreshToken, scope, ttl },
        _req,
        h
    ) => {
        console.log('clientId', clientId);
    ...
    .setDescription(
        'This API uses OAuth 2 with the authentication code grant flow. [More info](https://oauth.net/2/grant-types/authorization-code/)'
    )
    .setScopes({
