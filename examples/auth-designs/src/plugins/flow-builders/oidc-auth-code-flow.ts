...
export default OIDCAuthorizationCodeBuilder.create()
        ...
        route
            ...
            .generateCode(async ({ clientId, codeChallenge, scope, nonce }, { payload: { email, password } }) => {
                ...
                if (email == 'janed@example.com' && password == '1234') {
                    return {
                        type: 'code',
                        value: JSON.stringify({ clientId, codeChallenge, scope, nonce, user: '248289761001' }),
                    };
                }
                ...
                async (
                    {
                        ...
                        createIdToken,
                        code,
                        codeVerifier /*, verifyCodeVerifier*/,
                    },
                    _req
                ) => {
    ...
    .refreshTokenRoute((route) =>
        route.generateToken(
            async (
                { clientId, clientSecret, refreshToken, scope, ttl, tokenType, createJwtAccessToken, createIdToken },
                _req
            ) => {
                console.log('clientId', clientId);
                console.log('clientSecret', clientSecret);
                ...
                try {
                    ...
                    if (refreshToken === 'generated_refresh_token_from_ac' && createJwtAccessToken) {
                        ...
                        const newRefreshToken =
                            ...
                            'generated_refresh_token_from_ac';
                        return new OAuth2TokenResponse({ access_token: accessToken })
                            .setExpiresIn(ttl)
