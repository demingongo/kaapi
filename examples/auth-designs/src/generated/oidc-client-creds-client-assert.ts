...
import { BearerTokenType, ClientSecretJwt } from '@saurbit/oauth2';
import { createClientAssertionJwtVerify, createInMemoryKeyStore, decodeJwt, JoseJwksAuthority, JwksRotator } from "@saurbit/oauth2-jwt";
import {
...
export const oidcClientCredentialsFlow: KaapiOIDCClientCredentialsFlow = KaapiOIDCClientCredentialsFlowBuilder.create()
    ...
    .addClientAuthenticationMethod(
        new ClientSecretJwt(decodeJwt, createClientAssertionJwtVerify(({ clientId }, request) => {
            const url = new URL(request.url);
            return {
                audience: url.origin + '/token',
                issuer: clientId,
                subject: clientId,
            }
        }))
