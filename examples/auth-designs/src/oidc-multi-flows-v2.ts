...
export const OIDCMultiFlowsDesignV2 = MultipleFlowsBuilder.create()
    ...
    .add(
        OIDCAuthorizationCodeBuilder.create()
            .setTokenType(tokenType)
            .setTokenTTL(36000)
            .addClientAuthenticationMethod(new ClientSecretPost())
            .addClientAuthenticationMethod(new ClientSecretBasic())
            ...
            .authorizationRoute<object, { Payload: { email: string; password: string } }>((route) =>
                route
                    .setPath('/oauth2/v2/authorization')
                    .setClientId('testabc')
                    .setUsernameField('email')
                    .setPasswordField('password')
                    .generateCode(
                        async ({ clientId, codeChallenge, scope, nonce }, { payload: { email, password } }) => {
            ...
            .tokenRoute((route) =>
                route //.setPath('/oauth2/m2m/token')
                    .generateToken(
                        async (
                            { clientId, clientSecret, ttl, createJwtAccessToken, createIdToken, code, codeVerifier },
                            _req
                        ) => {
                            console.log('clientId', clientId);
                            console.log('clientSecret', clientSecret);
                            ...
                            try {
                                ...
                                if (createJwtAccessToken) {
                                    ...
                                    return new OAuth2TokenResponse({ access_token: accessToken })
                                        .setExpiresIn(ttl)
                                        .setRefreshToken(
                                            (scope?.split(' ').includes('offline_access') || undefined) && refreshToken
                                        )
                                        .setScope(scope?.split(' '))
                                        .setTokenType(tokenType)
                                        .setIdToken(idToken?.token);
            ...
            .refreshTokenRoute((route) =>
                route.validate(
                    async (
                        { clientId, clientSecret, refreshToken, scope, ttl, createJwtAccessToken, createIdToken },
                        _req,
                        h
                    ) => {
                        console.log('clientId', clientId);
    ...
    .add(
        OIDCDeviceAuthorizationBuilder.create()
            .setTokenType(tokenType)
            .setTokenTTL(36000)
            .addClientAuthenticationMethod(new NoneAuthMethod())
            //.jwksRoute(route => route.setPath('/oauth2/m2m/keys')) // activates jwks uri
            ...
            .tokenRoute((route) =>
                route //.setPath('/oauth2/m2m/token')
                    .generateToken(
                        async (
                            { clientId, deviceCode, clientSecret, ttl, createJwtAccessToken, createIdToken },
                            _req
                        ) => {
                            console.log('clientId', clientId);
                            console.log('clientSecret', clientSecret);
