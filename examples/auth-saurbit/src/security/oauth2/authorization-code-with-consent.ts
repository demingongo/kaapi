...
import {
  KaapiAuthorizationCodeFlow,
  KaapiAuthorizationCodeFlowBuilder,
...
export const authorizationCodeWithConsentFlow: KaapiAuthorizationCodeFlow<
  ReqRefDefaults,
  ParsedData,
  ReqRefDefaults
> = new KaapiAuthorizationCodeFlowBuilder<ReqRefDefaults, ParsedData>({
        ...
        try {
          ...
          if (session) {
            if (session.expiresAt <= Date.now()) {
              ...
              const user = REGISTERED_USERS.find((u) => u.id === session.userId);
              if (user) {
                const processedAuthorization = await authorizationCodeWithConsentFlow
                  .kaapi()
