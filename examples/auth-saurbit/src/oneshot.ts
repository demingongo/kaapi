...
const flow: KaapiOIDCAuthorizationCodeFlow<ReqRefDefaults, {
  ...
  parseAuthorizationEndpointData: async (req) => {
    ...
    const consent = typeof payload?.consent === "string" && ["allow", "deny"].includes(payload.consent) ?
      (payload.consent as "allow" | "deny") : undefined;
    const cookieSession = req.state[COOKIE_SESSION_NAME] && typeof req.state[COOKIE_SESSION_NAME] === "string" ?
      req.state[COOKIE_SESSION_NAME] : undefined;
  ...
  .getUserForAuthentication((_ctxt, parsedData) => {
    ...
    const user = AUTH_USERS.find((u) => u.username === parsedData.username && u.password === parsedData.password);
    if (!user) return;
    return {
      type: "authenticated",
      user: {
            ...
            if (session.expiresAt <= Date.now()) {
              ...
              if (user) {
                ...
                if (processedAuthorization.type === "continue") {
                  ...
                  sessionStorage.set(cookieSession, { ...session, expiresAt: Date.now() + COOKIE_SESSION_MAX_AGE });
...
app.route(
  applyModifiers<{
    ...
    options: {
      ...
      notes: "Returns claims about the authenticated user. Requires a valid access token with the 'openid' scope.",
      auth: {
...
app.route(
  applyModifiers<{
    ...
    responses: groupResponses(
      ...
      new ResponseDocsModifier()
        .setCode(403)
        .setDescription("Forbidden - valid access token but insufficient scope to access the resource."),
    )
  })
);
