...
| ----------------------------- | --------------------------- | ------- | ------------------------------------------------------------------------------------- |
| `packages/server`             | `@kaapi/server`             | 0.0.43  | Low-level Hapi.js wrapper with built-in `kaapi-auth` Bearer scheme                    |
| `packages/kaapi`              | `@kaapi/kaapi`              | 0.0.43  | **Main framework** — wraps server, adds logging, messaging, plugins, docs             |
| `packages/kafka-messaging`    | `@kaapi/kafka-messaging`    | 0.0.43  | KafkaJS-based `IMessaging` implementation                                             |
| `packages/oauth2-auth-design` | `@kaapi/oauth2-auth-design` | 0.0.43  | OAuth2/OIDC auth flows as Kaapi plugins (Auth Code, Client Credentials, Device, OIDC) |
| `packages/cli`                | `@kaapi/cli`                | 0.0.43  | Interactive `kaapi` CLI for scaffolding plugins, auth designs, etc.                   |
| `packages/validator-arktype`  | `@kaapi/validator-arktype`  | 0.0.43  | ArkType-based request validation plugin (ESM-only)                                    |
| `packages/validator-valibot`  | `@kaapi/validator-valibot`  | 0.0.43  | Valibot-based request validation plugin                                               |
| `packages/validator-zod`      | `@kaapi/validator-zod`      | 0.0.43  | Zod v4-based request validation plugin                                                |
| `packages/logger`             | `@kaapi/logger`             | 0.0.43  | Logger utilities for Kaapi                                                            |
